                    ...
                    _data.delete('checks', checkId, function(err){
                      ...
                      if(checksDeleted == checksToDelete) {
                        if(!deletionErrors){
                          ...
                          callback(500, {'Error': 'Error encountered while attempting to delete all of the user\'s checks. All checks may not have been deleted from system successfully'});
                        }
                      }
                    });
                  });
...
handlers._checks.post = function(data, callback) {
  ...
  if (protocol && url && method && successCodes && timeoutSeconds) {
    // Ge tthe token from the headers
    var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
      ...
      if(!err && tokenData) {
        ...
        _data.read('users', userPhone, function(err, userData) {
          if(!err && userData) {
            ...
            // Verify that the user have less checks than the maxChecks per user
    ...
    callback(400, {'Error' : 'Missing required field(s) or input are invalid'})
  }
};
              ...
              if(!err) {
                ...
                _data.read('users', checkData.userPhone, function(err, userData) {
                  if (!err && userData) {
                    ...
                    callback(500, {'Error': 'Could not find the user who created the user, so could remove the check from the list of checks on the user object'});
                  }
                });
              } else {
